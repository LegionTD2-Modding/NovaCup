    ...
    */
    var sorttable = {
        ...
        innerSortFunction: function (e) {
            ...
            var sorted = false; // LISKFIX to be perma descending
            var inverse = 1;  // LISKFIX to be perma descending
            var col = this['stCol'];
            var info = table['stInfo'];
            ...
            // LISK comment: this doesn't really seem to help make it more stable in actuality..
            //sorttable.shaker_sort(sortle.val, info[col].func);
            /* and comment out this one */
            sortle.val.sort(info[col].func);
            if (inverse && col > 0) sortle.val.reverse(); // LISKFIX to not reverse the first column
